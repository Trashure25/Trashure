...
export default function TrendingPage() {
  ...
  // Fetch trending listings (most recent as proxy for trending)
  useEffect(() => {
    const fetchListings = async (retryCount = 0) => {
      try {
        ...
        const fetchedListings = data.listings || data
        console.log('Fetched trending listings:', fetchedListings.length)
        
        setListings(fetchedListings)
        setFilteredListings(fetchedListings)
        
        // Extract unique brands for filter options
        const brands = [...new Set(fetchedListings.map(item => item.brand).filter(Boolean))]
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading trending items...</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Trending Items</h1>
        <p className="text-gray-600">Discover the most popular items getting the most views</p>
      </div>
                ...
                {filteredListings.map((item) => (
                  <ItemCard
                    ...
                    item={{
                      ...
                      userId: item.userId,
                      isNew: new Date(item.createdAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) // New if created within 7 days
                    }}
                  />
                ))}
              </div>
